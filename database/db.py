...
class Database():
    ...
    def createTables(self):
        ...
        createRetailers_Products = "CREATE TABLE Retailers_Products (product_id int, retailer_id int, name varchar(64), price varchar(32), msrp varchar(32), photo varchar(255), source varchar(255), instock varchar(32), new varchar(32), price_check varchar(64), product_type varchar(64), link varchar(2048), PRIMARY KEY (product_id, retailer_id), FOREIGN KEY (product_id) REFERENCES Products(product_id), FOREIGN KEY (retailer_id) REFERENCES Retailers(retailer_id))"
        ...
        try:
            for x in products:
                ...
                if not retailer:
                    ...
                    retailer = self.index("SELECT * FROM Retailers WHERE name='" + str(x.source) + "';")
                retID = retailer[0][0]
                query = "REPLACE INTO Retailers_Products (product_id, retailer_id, name, price, msrp, photo, source, instock, new, price_check, product_type, link) VALUES (" + str(prodID) + "," + str(retID) + ",'" + str(x.name) + "','"+ str(x.price).strip() +"','"+ str(x.msrp) +"','"+str(x.photo)+"','"+str(x.source)+"','"+str(x.instock).strip() +"','"+str(x.new)+"','"+str(x.price_check)+"','"+str(x.product_type)+"','"+str(x.product_link).strip()+"');"
                self.add(query)
        except Exception as e:
            print(e)
