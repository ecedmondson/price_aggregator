...
def get_compatibility():
    ...
    # sys.stdout.write("Checking if chromedriver installed...")
...
def install_chromedriver():
    ...
    # install_proc = subprocess.Popen(f"python chromedriver_installer/setup.py install --chromedriver-version=${sys.argv[2]}", shell=True, stdout=subprocess.PIPE).stdout
    # install_status = install_proc.read()
...
if sys.argv[1] not in ["--check", "--install"]:
    print(f"Unknown arg {sys.argv[1]} passed to bin/chrome_compatability. Typo?")
